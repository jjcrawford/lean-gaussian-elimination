...
namespace computable_matrix
    ...
    @[extensionality] protected lemma ext {a b : computable_matrix m n α} (h : ∀ i j, read a i j = read b i j) : a = b :=
    ...
    -- syntax : adds to the first row specified, s times the second row specified
    ...
    def multiply_row_by_scalar (a : computable_matrix m n α) [has_add α] [has_mul α] (i : fin m) (s : α) : computable_matrix m n α :=
    computable_matrix.write_row a i (array.map (has_mul.mul s) (a.read_row i))
    
    def multiply_col_by_scalar (a : computable_matrix m n α) [has_add α] [has_mul α] (j : fin n) (s : α) : computable_matrix m n α :=
    computable_matrix.write_col a j (array.map (has_mul.mul s) (a.read_col j))



    @[simp] lemma map₂_comm {f : α → α → α } {a b : computable_matrix m n α} {h : Π (i j : α), f i j = f j i} : map₂ f a b = map₂ f b a :=
    begin
        ext,
        unfold map₂,
        unfold foreach,
        ...
        unfold computable_d_matrix.read,
        have H : (iterate b b
            (λ (i : fin m) (j : fin n) (v : α) (a' : computable_matrix m n α),
               write a' i j (f (d_array.read (d_array.read a i) j) v))) = (iterate a a
         (λ (i : fin m) (j : fin n) (v : α) (a' : computable_matrix m n α),
            write a' i j (f (computable_d_matrix.read b i j) v))),

            show d_array.read _ _ = _,
            congr, from H,
